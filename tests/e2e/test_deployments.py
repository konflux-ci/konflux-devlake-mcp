...
@pytest.mark.parametrize("model", models)
@pytest.mark.flaky(max_runs=3)
async def test_llm_deployment_frequency_metrics(model, mcp_client):
    """Test that LLM can use get_deployment_frequency tool to analyze DORA metrics."""
    tools = await get_converted_tools(mcp_client)
    messages = [
        Message(role="system", content=DIRECTIVE_SYSTEM_PROMPT),
        Message(
            role="user",
            content=(
                "Get the deployment frequency metrics using the get_deployment_frequency tool. "
                "In your final answer include: the total number of deployments, "
                "the number of unique deployment days, the DORA performance level, "
                "and the average deployment days per week."
            ),
        ),
    ]
    expected_values = [
